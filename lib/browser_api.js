(function(window){
    window.debug = !!document.location.toString().match(/localhost/)
    ...
    var browser = {
        ...
        isFrame: typeof(window.pokki) != "object" && !!window.location.protocol.match(/^(http|file)/),
        ...
        isFirefox: !window.location.protocol.match(/^http/) && navigator.userAgent.match('Firefox') != undefined,
        ...
        getPageType: function(){
            ...
            if (browser.isChrome) {
                ...
                if (window.location.toString().match(/background\.html/))
                    browser.page_type = 'background';
                else
                    browser.page_type = 'popup';
            } else if (browser.isPokki) {
                browser.page_type = window.location.toString().match('background.html') ? 'background' : 'popup'
            }
        ...
        onReady: function(callback) {
            if (callback) {
                browser._onReadyCallbacks.push(callback);
                ...
                if (browser._isNetworkInitialized) {
                    ...
                    if (!browser.isFirefox || FFStore.synced) {
                        for (var i=0; i< browser._onReadyCallbacks.length; i++) {
                            browser._onReadyCallbacks[i](); 
                        }

                        browser._onReadyCallbacks = []
                    } else {
                        setTimeout(function() {
                            browser.onReady();
                        }, 50);
                ...
                if (browser._onReadyCallbacks.length) {
                    console.log("Calling onReady callback", browser._isNetworkInitialized);
                        ...
                        if (pokki.rpc('browser._isNetworkInitialized') === true) {
                            ...
                            document.addEventListener('click', function(evt) {
                                if (evt.target.nodeName == 'A' && evt.target.target == "_blank") {
                                    ...
                                    pokki.openURLInDefaultBrowser(evt.target.href);
        ...
        _frame_addMessageListener: function(listener) {
            if (!browser._bg_page && browser.page_type == 'popup') {
                ...
                browser._bg_page.id = "background_page";
                ...
                if (!browser._isNetworkInitialized) {
                    ...
                    setTimeout(function(){
                        window.parent.browser._bg_initialized();
                    }, 2000);
                }
            }
        ...
        _readLocalFile: function(file, callback, async){
            ...
            xhr.open('GET', browser.extension.getURL(file), async);
            xhr.send(null);
        ...
        loadConfiguration: function(callback){
            browser._readLocalFile("/manifest.json", function(response){
                ...
                store.set('_config', browser.config);
    ...
    document.getElementsByTagName('html')[0].className += " " + (function(){         
        return browser.isChrome ? "chrome" :
               ...
               browser.isPokki ? "pokki" :
               browser.isFrame ? "frame" : 
               "";
    }())
